...
class Therakles_project:
    ...
    def __init__(self):
        ...
        self.__consIDs = []                                  #ids of construction objects in database
    ...
    def initializeProject(self):
        ...
        tagClim.appendChild( rootTag.createTextNode("ISES_climate") )
    ...
    def submitZoneSettings(self, zoneObject):
        ...
        #check if model should be added or replaced
...
##--------------- Utilization Profiles: Equipment Loads Model from weekly schedule --------------------------------------------##
    ...
    def __createEmbDB(self, zoneObject ):
        ...
        try:
            ...
            #stores the combination of material and related identifier
            materialMapping = {}
            #loop over all material objects in zone Object
            if( len(zoneObject.return_MatList()) > 0 ):
                
                for materialObject in zoneObject.return_MatList():
                    currentIdentifier = self.__matIDs[counter]
                    ...
                    newMaterialTag.setAttribute("rho", str( values[4] ) )
                    newMaterialTag.setAttribute("cT", str( values[3] ) )
                    ...
                    materialMapping[values[1]] = currentIdentifier
            else:
            ...
            if( len(zoneObject.return_ConsList()) > 0 ):
                print len(zoneObject.return_ConsList())
                ...
                for consObject in zoneObject.return_ConsList():
                    ...
                    #adds the current IFC GUID to the CFD eKPI-File
                    
                    #loop over layers, create and fill layer tags
                    for layer in consObject.return_listOfLayers():
                        
                        #gets the matching material id from the current Therakles material list
                        currentMaterialId = materialMapping.get(layer.mat.return_name())
                        newLayerTag = self.__therakles_xml_tree.createElement("Layer")
                        newLayerTag.setAttribute("materialId", str( currentMaterialId))
            ...
            print("Error occurred while creating embedded object tags in therakles xml.")
            return False
