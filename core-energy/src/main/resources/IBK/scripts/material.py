...
class Material:
    ...
    def read_DelphinDataSet(self, stringFilePathAndName_m6 ):
        ...
        try:
            newFilePathAndName_m6 = stringFilePathAndName_m6
            print("Path of current material to be read: "+newFilePathAndName_m6)
            fileObject = open( newFilePathAndName_m6, "r" )
        except Exception as e:
            print("Error while parsing material file %s." % newFilePathAndName_m6)
            print(e)
            return False
        else:
            lineCounter = 0
            for line in fileObject.readlines():
                ...
                if( line.find("=") > 0 ): 
                    ...
                    rowItems = line.split()
                    valueParameter = rowItems[0]
                    valueValue = rowItems[2]
                    valueUnit = rowItems[3]
                    ...
                    if(valueParameter.__contains__("RHO")):
                        self.__rho = valueValue
                    elif(valueParameter.__contains__("NAME")):
                        name = valueValue+valueUnit
                        name = name.replace("EN:","")
                        name = name.replace("DE:","")
                        self.__name = name
                    elif(valueParameter.__contains__("CE")):
                        self.__ce = valueValue
                    elif(valueParameter.__contains__("OPOR")):
                        self.__opor = valueValue
                    elif(valueParameter.__contains__("OEFF")):
                        self.__oeff = valueValue
                    elif(valueParameter.__contains__("MEW")):
                        self.__mew = valueValue
                    elif(valueParameter.__contains__("AW")):
                        self.__aw = valueValue
                    elif(valueParameter.__contains__("KLEFF")):
                        self.__kleff = valueValue
                    elif(valueParameter.__contains__("LAMBDA")):
                        self.__la = valueValue
                    elif(valueParameter.__contains__("OCAP")):
                        self.__ocap = valueValue
                    elif(valueParameter.__contains__("O80")):
                        self.__o80 = valueValue
