...
class Nandrad_xml:
    ...
    def __init__(self):
        ...
        self.__pathConstructions = ""                   #complete absolute folder of construction files containing d6p files        self.__pathTemplates = ""                       #complete absolute folder of remaining files (schedules, spacetypes, outputs)
    ...
    def init_file(self, nandrad_xml): 
        ...
        try:
            self.__nandrad_xml_tree = DOM.parse( nandrad_xml.decode('latin-1').encode('utf-8'))
        
        except Exception:
            
            self._nandrad_xml_initialized = False
            print("Error during reading the Nandrad project file: ")
            return False
            ...
            #get file location
            self.__projectFileName = "ISES_project.xml"  #index of last element equals length of list - 1
            ...
            self.__projectDirectory = ProjectFolder
            print(self.__projectDirectory)
    ...
    def __init_Zones(self):
        ...
        for phTag in self.__nandrad_xml_tree.getElementsByTagName("Placeholder"):
            ...
            if( nameAtt == "Climate Database" ):
                ...
                self.__pathTemplates = os.path.dirname(os.path.realpath(__file__))
                newTemplatesPath = os.path.join(self.__pathTemplates, "project", "DB_Templates")
                self.__pathTemplates = newTemplatesPath
                noIdentLoc += 1
        ...
        for zoneTag in zoneTagList:
            ...
            zoneIFC = zoneTag.getAttribute("fromElement")
            
            self.__zoneIFCKeys.append( zoneIFC )
            self.__zoneIndizes.append( zoneID )                     
            
            self.__zoneIdDict[zoneIFC] = zoneID
        ...
        if ( self._nandrad_xml_initialized == False):
            ...
            for zoneNode in self.__nandrad_xml_tree.getElementsByTagName("Zone"):
                ...
                if ( int(nandradZoneId) == int(zoneId) ):
                    ...
                    zoneSource = zoneNode.getAttribute("fromElement")
                    ...
                    firstPos = zoneDisplayName.find("{")
                    zoneName = zoneDisplayName[0:(firstPos)]
                    ...
                    for constrNode in self.__nandrad_xml_tree.getElementsByTagName("ConstructionInstance"):
                        ...
                        for idNode in constrNode.getElementsByTagName("ZoneID"):
                            ...
                            if( int(refZoneId) == int(zoneId)):
                                ...
                                newConstruction = Construction("Construction_"+c_ifc)
                                ...
                                if( tag_Rse.getAttribute("name") == "HeatTransferCoefficient"):
                                    ...
                                    print("Heat transfer coefficient: "+c_Rse)
                                ...
                                for consTypeRefTag in self.__nandrad_xml_tree.getElementsByTagName("ConstructionTypeReference"):
                                    ...
                                    nameAsFilename = consTypeRefTag.firstChild.nodeValue
                                    nameAsFilename = nameAsFilename.replace("${Construction Database}/", "")
                                    nameAsFilename = nameAsFilename.replace("${Construction Database}\\", "")
                                    nameWithID = consTypeRefTag.getAttribute("displayName")
                                    
                                    if( (consTypeRefTag.firstChild.nodeValue.__contains__("_"+c_id+".d6p")) and (consTypeRefTag.firstChild.nodeValue.__contains__(nameWithID))):
                                        
                                        print("Fitting construction in constr. type references found.")
                                        #newPathWithSep = self.__pathConstructions + os.sep
                                        newPathOfScript = os.path.dirname(os.path.realpath(__file__))
                                        print newPathOfScript
                                        newPathOfConstructions = os.path.join(os.path.dirname(newPathOfScript), "project", "DB_Constructions", nameAsFilename)
                                        ...
                                        self.__pathMaterials = os.path.dirname(os.getcwd())+os.sep+"project"+os.sep+"DB_Materials"
                                        
                                        if (newConstruction.read_DelphinDataSet( newPathOfConstructions, self.__pathMaterials ) == True):
                                            flag=1
                    ...
                    #no direct definition, check for referenced definition
                        ...
                        placeHolderAlt = "${Templates}\\"
                        placeHolderInside = ""
                        onePlaceHolderInside = False
                        
                        if (placeHolder in referencePath):
                            
                            onePlaceHolderInside = True
                            placeHolderInside = placeHolder
                        
                        elif(placeHolderAlt in referencePath):
                            
                            onePlaceHolderInside = True
                            placeHolderInside = placeHolderAlt
                        
                        if(( self.__pathsDefined == True ) and ( onePlaceHolderInside)):
                            
                            referencePath = referencePath.replace( placeHolderInside, "")
                            spaceTypes = os.path.join(self.__pathTemplates, referencePath)
                            ...
                            try:
                                print("referencePath: "+spaceTypes)                        
                                spaceType_xml = DOM.parse( spaceTypes )
                            except Exception as e:
                                print("Error while parsing space type definition file %s. Check file format of referenced xml." % spaceTypes)
                                print(e)
                            else:
                                for spaceTypeNode in spaceType_xml.getElementsByTagName("SpaceType"):
                                    if(spaceTypeNode.getAttribute("name") == spaceTypeName):
                                        print("found matching space type")
                                        #definition for desired zone was found, see if we have heating/cooling model
                                        for node in spaceTypeNode.getElementsByTagName("IBK:String"):
                                            ...
                                            if(node.getAttribute("name") == "HeatingControlMode"):
                                                ...
                                                print("Couldn't interprete given Template path %s." % spaceTypes)
                    ...
                    #no direct definition, check for referenced definition
                        ...
                        schedulesName = referencePath.replace("${Templates}/", "")
                        newPathProjectTemplates = newPathParent+os.sep+"project"+os.sep+"DB_Templates"+os.sep+schedulesName
                        print("Schedules path: "+newPathProjectTemplates)
                        
                        if( self.__pathsDefined == True ) and ( placeHolder in referencePath):
                            #now open file and try to get relevant space type definition from xml (parse xml)
                            try:                        
                                schedules_xml = DOM.parse( newPathProjectTemplates )
                                ...
                                for spaceTypeGroupNode in schedules_xml.getElementsByTagName("SpaceTypeGroup"):
                                    ...
                                    if(spaceTypeGroupNode.getAttribute("spaceTypeName") == spaceTypeName):
                                        ...
                                        print("space type group: "+spaceTypeGroupNode.getAttribute("spaceTypeName"))
                                        #read schedules and submit schedules to zone object settings
                                        ScheduleObjectHeatingSetPoints = self.__read_NandradWeekSchedule( spaceTypeGroupNode, "HeatingSetPointTemperature", DataType.TEMPERATURE, weekEndDaysList)
                                        newZoneObject.add_idealHeatingModel(heat_pmax, heat_mode, heat_conType, ScheduleObjectHeatingSetPoints)
